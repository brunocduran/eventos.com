...
public class OrganizadorEventoCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            OrganizadorEvento oOrganizadorEvento = (OrganizadorEvento) dao.carregar(idOrganizadorEvento);
            
            Gson ogson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();
            String jSon = ogson.toJson(oOrganizadorEvento);
            response.getWriter().write(jSon);
        } catch (Exception ex) {
            System.out.println("Erro no servlet ao organizadoreventocarregar: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
