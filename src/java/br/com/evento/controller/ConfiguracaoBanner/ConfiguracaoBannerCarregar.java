 ...
 * @author johat
 */
@WebServlet(name = "ConfiguracaoBannerCarregar", urlPatterns = {"/ConfiguracaoBannerCarregar"})
public class ConfiguracaoBannerCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            ConfiguracaoBanner oCba = (ConfiguracaoBanner) dao.carregar(idConfiguracaoBanner);
            
            Gson ogson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();
            String jSon = ogson.toJson(oCba);
            response.getWriter().write(jSon);
        }catch(Exception ex){
            System.out.println("Erro ao buscar Configuracao de Banner - "+ex.getMessage());
            ex.printStackTrace();
        }
    }
