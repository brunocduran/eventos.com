 ...
 */
package br.com.evento.controller.cidade;
...
public class CidadeCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Cidade oCidade = (Cidade) dao.carregar(idCidade);

            Gson ogson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();
            String jSon = ogson.toJson(oCidade);
            response.getWriter().write(jSon);
        } catch (Exception ex) {
            System.out.println("Erro no servlet ao carregar cidade: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
