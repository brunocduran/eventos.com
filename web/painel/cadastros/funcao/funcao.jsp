                    ...
                    <table id="datatable" class="display">
                        ...
                        <tbody>
                            <c:forEach var="funcao" items="${funcoes}">
                                <tr>
                                    <td align="right">${funcao.idFuncao}</td>
                                    ...
                                    <td align="center">
                                        <a href="#" onclick="deletar(${funcao.idFuncao}, '${funcao.situacao}')">
                                            <button class="btn
                                                    <c:out value="${funcao.situacao == 'A' ? 'btn-danger':'btn-success'}"/>">
                                                <i class="fas fa-fw
                                                   <c:out value="${funcao.situacao == 'A' ? 'fa-times' : 'fas fa-check'}"/>"></i>
                                                <Strong>
                                                    <c:out value="${funcao.situacao == 'A' ? 'Inativar' : 'Ativar'}"/>
                                                </Strong>
                                            </button></a>
                                    </td>
                                </tr>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <input class="form-control" type="hidden" name="idfuncao" id="idfuncao" value="" readonly="readonly"/>
...
<script>
    ...
    function setDadosModal(valor) {
        ...
        var idFuncao = valor;
        if (idFuncao != "0") {
            $.getJSON('FuncaoCarregar', {idFuncao: idFuncao}, function (respostaServlet) {
    ...
    function deletar(codigo, situacao) {
        ...
        if (situacao == 'I') {
            titulo = "Você deseja realmente ativar a função?";
            confirmButtonText = "Sim, ative a função!";
            tituloConfirmacao = "Função ativada com sucesso!";

        } else {
            titulo = "Você deseja realmente inativar a função?";
            confirmButtonText = "Sim, inative a função!";
            tituloConfirmacao = "Função inativada com sucesso!";
        }
                            ...
                            function (data) {
                                if (data == 1) {
                                    ...
                                    Swal.fire({
                                        ...
                                        icon: 'error',
                                        title: 'Erro',
                                        text: 'Não foi possível ativar/inativar a Função!',
                                        showConfirmButton: true,
                                        timer: 10000
                                    }).then(function () {
                                        window.location.href = "${pageContext.request.contextPath}/FuncaoListar";
