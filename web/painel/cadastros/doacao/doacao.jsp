...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            ...
            <c:if test="${eventoCarregadoDoacao.idEvento != null}">
                <p class="mb-4">Planilha de Registros - <b>Doações do Evento:
                        ${eventoCarregadoDoacao.nomeEvento} </b></p>
                <a href="${pageContext.request.contextPath}/EventoGerenciar?idEvento=${eventoCarregadoDoacao.idEvento}"
            ...
            <a href="#modaladicionar" class="btn btn-success mb-4 adicionar" data-toggle="modal" data-ad=""
                        ...
                        <table id="datatable" class="display">
                            ...
                            <tbody>
                                <c:forEach var="doacao" items="${doacoes}">
                                    <tr>
                                        <td align="right">${doacao.idDoacao}</td>
                                        <td align="left">
                                            <fmt:formatNumber value="${doacao.valorDoacao}"
                                                              type="currency" />
                                        </td>
                                        <td align="left">
                                        ...
                                        <td align="left">${doacao.patrocinador.nomeRazaoPessoa}</td>
                                        <td align="left">${doacao.evento.nomeEvento}</td>
                                        <td align="center">
                                            <a class="btn btn-group-lg btn-primary" data-toggle="modal" data-id=""
                                               onclick="alterar(${doacao.idDoacao}, '${doacao.situacao}')">
                                                <i class="fas fa-edit"></i><Strong> Alterar </Strong> </a>
                                        </td>

                                        <td align="center">
                                            <a href="#"
                                               onclick="alterarStatus(${doacao.idDoacao}, '${doacao.situacao}')">
                                                <button class="btn
                                                        <c:out value=" ${doacao.situacao=='A' ? 'btn-success'
                                                                         :'btn-warning'}" />">
                                                    <i class="fas fa-fw
                                                       <c:out value=" ${doacao.situacao=='A' ? 'fas fa-check'
                                                                        : 'fa-times' }" />"></i>
                                                    <Strong>
                                                        <c:out
                                                            value="${doacao.situacao == 'A' ? 'Baixar' : 'Estornar'}" />
                                                    </Strong>
                                                </button></a>
                                        </td>
                                        ...
                                        <td align="center">
                                            <a href="#"
                                               onclick="deletar(${doacao.idDoacao}, '${doacao.situacao}')"
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <input class="form-control" type="hidden" name="idDoacao" id="idDoacao"
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="date" name="dataDoacao" id="dataDoacao"
                                       value="${doacao.dataDoacao}" maxLength="8" />
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="idPatrocinador" id="idPatrocinador"
                                        required>
                                    <option value="nulo">Selecione</option>
                                    <c:forEach var="patrocinador" items="${patrocinadores}">
                                        <option value="${patrocinador.idPatrocinador}"
                                                ${Doacao.patrocinador.idPatrocinador==patrocinador.idPatrocinador
                                                  ? "selected" : "" }>
                                        ...
                                        <select class="form-control" name="idEvento" id="idEvento" required>
                                            ...
                                            <c:forEach var="evento" items="${eventos}">
                                                <option value="${evento.idEvento}"
                                                        ${Doacao.evento.idEvento==evento.idEvento ? "selected" : ""
                                                        }>
                                                    ${evento.nomeEvento}
                                                </option>
                                            </c:forEach>
                                ...
                                <c:if test="${eventoCarregadoDoacao.idEvento != null}">
                                    <input class="form-control" type="hidden" name="idEvento" id="idEvento"
                                           value="${eventoCarregadoDoacao.idEvento}" readonly="readonly" />
                                </c:if>
        ...
        function alterar(valor, situacao) {
            ...
            if (status == 'P') {
                Swal.fire({
                    position: 'center',
                    icon: 'error',
                    title: 'Só é possível alterar doações que não foram baixadas!',
...
function deletar(codigo, situacao) {
    ...
    if (status == 'P') {
        Swal.fire({
            position: 'center',
            icon: 'error',
            title: 'Só é possível excluir doações que não foram baixadas!',
            showConfirmButton: true
        });
    } else {
